...
proc filepool {args} {
	...
	switch -- $cmd {
		...
		"default" {
			error "Subcomando invalido, esperaba del tipo 'list add remove reset', obtuve '$cmd'"
		}
	}
}
...
proc filepool_checktypes {types} {
	...
	foreach type $types {
		if {$type ni $valid} {
			error "Tipo invalido, esperaba del tipo '$valid', obtuve '$type'"
		}
	}
}
...
proc filepool_add {args} {
	...
	foreach {name value} $args {
		if {$name eq "-position"} {
			...
			error "Opcion desconocida: $name"
		}
	}
	if {($pos < 0) || ($pos > [llength $::__filepool])} {
		error "Valor fuera de rango: [expr {$pos + 1}]"
	}
	if {$path eq ""} {
		error "Falta -path"
	}
